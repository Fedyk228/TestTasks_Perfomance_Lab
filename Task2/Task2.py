...
def check_points_in_circle(data_circle, data_points):
    def get_circle_data(data_circle):
        ...
        circle_x, circle_y = circle_data[0][0], circle_data[0][1]
        circle_rx = circle_data[1][0]
        circle_ry = circle_data[1][1]
        return circle_x, circle_y, circle_rx, circle_ry
    ...
    def point_is_circle(data_circle, data_points, i):
        c_x, c_y, r_x, r_y = get_circle_data(data_circle)
        p_x, p_y = get_point_data(data_points, i)

        ellipse_value = ((p_x - c_x) / r_x) ** 2 + ((p_y - c_y) / r_y) ** 2

        if abs(ellipse_value - 1) < 1e-10:
            print("0")
